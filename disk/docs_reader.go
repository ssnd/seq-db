...
func NewDocsReader(reader *Reader, file *os.File, docsCache *cache.Cache[[]byte]) *DocsReader {
	return &DocsReader{
		reader: reader,
		File:   file,
		Cache:  docsCache,
...
func (r *DocsReader) getDocBlockLen(offset int64) (uint64, error) {
	...
	n, err := r.reader.ReadAt(r.File, buf.B, offset)
	if err != nil {
		return uint64(n), err
	}
...
func (r *DocsReader) ReadDocBlock(offset int64) ([]byte, uint64, error) {
	...
	buf := make([]byte, l)
	n, err := r.reader.ReadAt(r.File, buf, offset)
